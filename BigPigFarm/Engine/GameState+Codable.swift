...
extension CodableSnapshot {
    init(from decoder: Decoder) throws {
        ...
        guineaPigs = try container.decode([UUID: GuineaPig].self, forKey: .guineaPigs)
        facilities = try container.decode([UUID: Facility].self, forKey: .facilities)
        farm = try container.decode(FarmGrid.self, forKey: .farm)
        money = try container.decode(Int.self, forKey: .money)
        gameTime = try container.decode(GameTime.self, forKey: .gameTime)
        ...
        lastSave = try container.decodeIfPresent(Date.self, forKey: .lastSave)
        events = try container.decode([EventLog].self, forKey: .events)
        pigdex = try container.decode(Pigdex.self, forKey: .pigdex)
        contractBoard = try container.decode(ContractBoard.self, forKey: .contractBoard)
        breedingProgram = try container.decode(BreedingProgram.self, forKey: .breedingProgram)
        breedingPair = try container.decodeIfPresent(BreedingPair.self, forKey: .breedingPair)
        socialAffinity = try container.decode([String: Int].self, forKey: .socialAffinity)
        farmTier = try container.decode(Int.self, forKey: .farmTier)
        purchasedUpgrades = try container.decode(Set<String>.self, forKey: .purchasedUpgrades)
        totalPigsBorn = try container.decode(Int.self, forKey: .totalPigsBorn)
        totalPigsSold = try container.decode(Int.self, forKey: .totalPigsSold)
        totalEarnings = try container.decode(Int.self, forKey: .totalEarnings)
        boosterState = try container.decodeIfPresent(BoosterState.self, forKey: .boosterState) ?? BoosterState()
        completedMilestones = try container.decodeIfPresent(Set<MilestoneID>.self, forKey: .completedMilestones) ?? []
    }
}
